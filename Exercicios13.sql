use master
drop database exerc13
create database exerc13 
go
use exerc13
go
...
SELECT e.nome, SUM(v.distanciaPercorrida) AS totaldistância,  CAST(AVG(v.distanciaPercorrida) AS DECIMAL(10, 2)) AS media_dist
FROM empresa e
INNER JOIN carro c on c.idEmpresa = e.id
INNER JOIN viagem v on v.idCarro = c.id
GROUP BY e.nome
...
SELECT e.nome, COUNT(c.id) AS quantidade
FROM empresa e
INNER JOIN carro c on c.idEmpresa = e.id
GROUP BY e.nome
HAVING COUNT(c.id) >= 3
...
SELECT e.nome
FROM empresa e
LEFT JOIN carro c on c.idEmpresa = e.id
WHERE c.id IS NULL
...
FROM carro c
LEFT JOIN viagem v on v.idCarro = c.id
...
SELECT c.marca, c.modelo, e.nome, COUNT(v.idCarro) AS viagens
FROM carro c
INNER JOIN viagem v on v.idCarro = c.id
INNER JOIN empresa e on e.id = c.idEmpresa
...
CASE
WHEN v.distanciaPercorrida < 1000 THEN 'R$10'
	ELSE 'R$' + CAST(LEFT(v.distanciaPercorrida / 1000, 1) * 15 AS VARCHAR)
END AS valor
FROM empresa e
INNER JOIN carro c on c.idEmpresa = e.id
INNER JOIN viagem v on v.idCarro = c.id
